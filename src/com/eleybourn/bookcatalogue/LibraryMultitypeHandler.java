...
import com.eleybourn.bookcatalogue.booklist.LibraryRowView;
 ...
 */
public class LibraryMultitypeHandler implements MultitypeListHandler {
    ...
    public int getItemViewType(Cursor c) {
        LibraryRowView rowView = ((BooklistSupportProvider) c).getRowView();
     ...
     */
    private BooklistHolder newHolder(LibraryRowView rowView) {
    ...
    public View getView(Cursor c, LayoutInflater inflater, View convertView, ViewGroup parent) {
        final LibraryRowView rowView = ((BooklistSupportProvider) c).getRowView();
     ...
     */
    public void buildContextMenu(CatalogueDBAdapter mDb, LibraryRowView rowView, ArrayList<SimpleDialogItem> menu) {
        try {
    ...
    private String getAuthorFromRow(CatalogueDBAdapter db, LibraryRowView rowView) {
        String author = null;
    ...
    private String getSeriesFromRow(CatalogueDBAdapter db, LibraryRowView rowView) {
        String series = null;
     ...
     */
    public boolean onContextItemSelected(CatalogueDBAdapter db, LibraryRowView rowView, final Activity context, final CatalogueDBAdapter dba, final int itemId) {
        if (itemId == R.id.MENU_DELETE_BOOK) {
        ...
        } else if (itemId == R.id.MENU_DELETE_SERIES) {
            long id = rowView.getSeriesId();
     ...
     * @author Philip Warner
     */
    public abstract static class BooklistHolder extends MultitypeHolder<LibraryRowView> {
        /**
        ...
        public static int getDefaultLayoutId(final int level) {
            ...
            switch (level) {
                case 1:
                    id = R.layout.library_row_level_1;
                    break;
                case 2:
                    id = R.layout.library_row_level_2;
                    break;
                default:
                    id = R.layout.library_row_level_3;
                    break;
            }
            return id;
        }
    ...
    public static class BookHolder extends BooklistHolder {
        ...
        @Override
        public void map(LibraryRowView rowView, View v) {
            final LibraryStyle style = rowView.getStyle();
            ...
            if ((extras & LibraryStyle.EXTRAS_THUMBNAIL) != 0) {
            ...
            shelves = v.findViewById(R.id.field_shelves);
            if ((extras & LibraryStyle.EXTRAS_BOOKSHELVES) != 0) {
            ...
            if ((extras & LibraryStyle.EXTRAS_AUTHOR) != 0) {
            ...
            if ((extras & LibraryStyle.EXTRAS_LOCATION) != 0) {
            ...
            if ((extras & LibraryStyle.EXTRAS_PUBLISHER) != 0) {
        ...
        @Override
        public void set(LibraryRowView rowView, View v, final int level) {
            ...
            if ((extras & LibraryStyle.EXTRAS_THUMBNAIL) != 0)
                rowView.getUtils().fetchBookCoverIntoImageView(cover, rowView.getMaxThumbnailWidth(), rowView.getMaxThumbnailHeight(), true, rowView.getBookUuid(),
                        AdminLibraryPreferences.isThumbnailCacheEnabled(), AdminLibraryPreferences.isBackgroundThumbnailsEnabled());
        ...
        @Override
        public View newView(LibraryRowView rowView, LayoutInflater inflater, ViewGroup parent, final int level) {
            // All book rows have the same type of view.
            return inflater.inflate(R.layout.library_row, parent, false);
        }
    }
    ...
    private static class GetBookExtrasTask implements SimpleTask {
        ...
        public void onFinish(Exception e) {
            synchronized (mHolder) {
                ...
                if ((mFlags & LibraryStyle.EXTRAS_BOOKSHELVES) != 0 && !mShelves.isEmpty())
                    mHolder.shelves.setText(mShelves);
                if ((mFlags & LibraryStyle.EXTRAS_AUTHOR) != 0 && !mAuthor.isEmpty())
                    mHolder.author.setText(mAuthor);
                if ((mFlags & LibraryStyle.EXTRAS_LOCATION) != 0 && !mLocation.isEmpty())
                    mHolder.location.setText(mLocation);
                if ((mFlags & LibraryStyle.EXTRAS_PUBLISHER) != 0 && !mPublisher.isEmpty())
    ...
    public static class GenericStringHolder extends BooklistHolder {
         ...
         */
        private GenericStringHolder(LibraryRowView rowView, DomainDefinition domain, int noDataId) {
    ...
    public static class MonthHolder extends BooklistHolder {
        ...
        public MonthHolder(LibraryRowView rowView, String source) {
            // Source column name
    ...
    public static class RatingHolder extends BooklistHolder {
        ...
        public RatingHolder(LibraryRowView rowView, String source) {
            // Source column name
            mSourceCol = rowView.getColumnIndex(source);
        }
