...
public class LibraryBuilder {
    /**
     * Counter for LibraryBuilder IDs
     */
     ...
     */
    private final CursorFactory mBooklistCursorFactory = (db, masterQuery, editTable, query) -> new BooklistCursor(db, masterQuery, editTable, query, LibraryBuilder.this, CatalogueDBAdapter.getSynchronizer());
    /**
     ...
     */
    private ArrayList<SynchronizedStatement> mLevelBuildStmt = null;
    ...
    @NonNull
    private static String getString(SummaryBuilder summary, boolean collationIsCs) {
     ...
     */
    public void requireDomain(DomainDefinition domain, String sourceExpression, boolean isSorted) {
        // Save the details
        ...
        if (mExtraDomains.containsKey(domain.name)) {
            ...
            if (oldInfo.sourceExpression == null) {
                if (info.sourceExpression == null) {
                    ...
                    ok = info.sourceExpression.isEmpty();
                }
            } else {
                if (info.sourceExpression == null) {
    ...
    public void rebuild() {
        ...
        for (SynchronizedStatement s : mLevelBuildStmt)
            s.execute();
    }
        ...
        try {
            ...
            for (LibraryGroup g : mStyle) {
                ...
                switch (g.kind) {
                    ...
                    // NEW_KIND: Add new kinds to this list
            ...
            //
            // The seriesLevel and authorLevel fields will influence the nature of the join. If at a later
            ...
            String sortColNameList = getString(summary, collationIsCs);
     ...
     *
     * @return    ArrayList	Array of row details, including absolute positions and visibility. Null if not present
     */
    ...
    private class SummaryBuilder {
        ...
        public SqlComponents buildSqlComponents(CompoundKey rootKey) {
            ...
            StringBuilder values = getStringBuilder(columns, expressions);
        ...
        @NonNull
        private StringBuilder getStringBuilder(StringBuilder columns, StringBuilder expressions) {
